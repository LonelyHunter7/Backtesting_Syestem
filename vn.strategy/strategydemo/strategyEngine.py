...
#         "Date","Time","Open","High","Low","Close","TotalVolume"
        ...
        if symbol in self.__dictSymbolStrategy:
            ...
            tick.lastPrice = data['Close']
             
            tick.volume = data['TotalVolume']
            tick.date= data['Date']
            tick.time = data["Date"]+u" "+data['Time']
#             print(tick.time)
...
#             tick.askVolume5 = data['AskVolume5']   
    ...
    #----------------------------------------------------------------------
    def sendOrder(self, symbol, direction, offset, price, volume, time, strategy):
        """
        发单（仅允许限价单）
        symbol：合约代码
...
class StrategyTemplate(object):
    ...
    #----------------------------------------------------------------------
    def buy(self, price,volume,time,stopOrder=False):
        """买入开仓"""
        if self.trading:
            if stopOrder:
                ...
                ref = self.engine.sendOrder(self.symbol, DIRECTION_BUY,
                                              OFFSET_OPEN, price, volume,time, self)
                return ref
        else:
            return None
    
    #----------------------------------------------------------------------
    def cover(self, price, volume,time, stopOrder=False):
        """买入平仓"""
        if self.trading:
            if stopOrder:
                so = self.engine.placeStopOrder(self.symbol, DIRECTION_BUY,
                ...
                ref = self.engine.sendOrder(self.symbol, DIRECTION_BUY,
                                              OFFSET_CLOSE, price, volume, time, self)
                return ref
        else:
            return None
    
    #----------------------------------------------------------------------
    def sell(self, price, volume,time, stopOrder=False):
        """卖出平仓"""
        if self.trading:
            if stopOrder:
    ...
    #----------------------------------------------------------------------
    def short(self, price, volume,time, stopOrder=False):
        """卖出开仓"""
        if self.trading:
            if stopOrder:
                so = self.engine.placeStopOrder(self.symbol, DIRECTION_SELL,
